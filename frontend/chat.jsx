...
const RESPONSES = {
  resume: {
    "هوش مصنوعی": {
      md: `بر اساس پایگاه دانش رزومه‌ی اعضا، **۷ نفر** با تخصص هوش مصنوعی شناسایی شدند:

- **علی محمدی‌فرد** — مهندس یادگیری ماشین · ۶ سال تجربه · تمرکز روی *پردازش زبان طبیعی فارسی*
- **سارا احمدی** — پژوهشگر بینایی ماشین · فارغ‌التحصیل دانشگاه گیلان · مقاله‌ی منتشرشده در حوزه‌ی *تشخیص اشیا*
- **رضا حسینی** — توسعه‌دهنده‌ی مدل‌های زبانی · سابقه‌ی همکاری با دو استارتاپ فعال در حوزه‌ی \`LLM\`

برای مشاهده‌ی فهرست کامل، می‌توانید نام یک حوزه‌ی فرعی را بپرسید.`,
      cites: [
        { title: "رزومه‌ی علی محمدی‌فرد", loc: "members/ali-mohammadi.pdf · p.2", score: "0.91" },
        { title: "رزومه‌ی سارا احمدی", loc: "members/sara-ahmadi.pdf · p.1", score: "0.88" },
        { title: "رزومه‌ی رضا حسینی", loc: "members/reza-hosseini.pdf · p.3", score: "0.84" }
      ]
    },
    "بک‌اند": {
      md: `**۴ نفر** با تجربه‌ی بیش از ۵ سال در توسعه‌ی بک‌اند پیدا شد:

- **مهدی کریمی** — ۸ سال · پشته‌ی \`Python / Django\` · معماری میکروسرویس
- **نگار رضوی** — ۶ سال · پشته‌ی \`Node.js\` · طراحی APIهای \`REST\` و \`GraphQL\`
- **امیر طاهری** — ۷ سال · پشته‌ی \`Go\` · تجربه در سامانه‌های با بار بالا

پیشنهاد می‌شود برای پروژه‌های با حساسیت بالا، با **مهدی کریمی** یا **امیر طاهری** هماهنگ شود.`,
      cites: [
        { title: "رزومه‌ی مهدی کریمی", loc: "members/mehdi-karimi.pdf · p.2", score: "0.93" },
        { title: "رزومه‌ی نگار رضوی", loc: "members/negar-razavi.pdf · p.1", score: "0.86" },
        { title: "رزومه‌ی امیر طاهری", loc: "members/amir-taheri.pdf · p.4", score: "0.82" }
      ]
    },
    "همکاری": {
      md: `بر اساس بخش *تجربه‌ی کاری* رزومه‌ها، **۱۲ عضو** سابقه‌ی همکاری مستقیم با شرکت‌های فناور را اعلام کرده‌اند. سه نمونه‌ی شاخص:

- **پویا صبوری** — همکاری ۲ ساله با شرکت دانش‌بنیان فناوران رشت در حوزه‌ی \`IoT\`
- **مریم نوری** — مشارکت در تیم تحقیق و توسعه‌ی یک شتاب‌دهنده‌ی فین‌تک
- **حسین یوسفی** — توسعه‌ی محصول برای استارتاپ سلامت دیجیتال

اگر صنعت یا فناوری مشخصی مدنظر دارید بفرمایید تا فهرست دقیق‌تری ارائه شود.`,
      cites: [
        { title: "رزومه‌ی پویا صبوری", loc: "members/pooya-sabouri.pdf · p.2", score: "0.81" },
        { title: "رزومه‌ی مریم نوری", loc: "members/maryam-nouri.pdf · p.3", score: "0.79" }
      ]
    },
    "طراحی محصول": {
      md: `**۳ متخصص** در حوزه‌ی طراحی محصول و تجربه‌ی کاربری:

- **آرش معینی** — ۹ سال طراحی محصول · سرپرستی تیم \`UX\` در دو محصول \`B2C\`
- **لیلا اسدی** — متخصص پژوهش کاربر · مسلط به روش‌شناسی *Jobs-to-be-Done*
- **سینا فلاح** — طراح رابط کاربری · تخصص در طراحی سیستم‌های طراحی (\`Design Systems\`)

برای مشاهده‌ی نمونه‌کارها، نام عضو موردنظر را وارد کنید.`,
      cites: [
        { title: "رزومه‌ی آرش معینی", loc: "members/arash-moeini.pdf · p.1", score: "0.90" },
        { title: "رزومه‌ی لیلا اسدی", loc: "members/leila-asadi.pdf · p.2", score: "0.87" }
      ]
    }
  },
  rules: {
    "پذیرش": {
      md: `بر اساس **آیین‌نامه‌ی پذیرش مرکز رشد گیلان** (مصوب شورای مرکز)، شرایط اصلی عبارت‌اند از:

- داشتن طرح فناورانه یا نوآورانه‌ی قابل دفاع در یکی از حوزه‌های اولویت‌دار
- ارائه‌ی برنامه‌ی کسب‌وکار اولیه (\`BMC\`) به‌همراه تحلیل بازار
- حضور حداقل یکی از مؤسسان به‌صورت **تمام‌وقت** در مرکز
- عدم اشتغال متقاضی به‌صورت دائم در ارگان‌های دولتی

روند پذیرش شامل سه مرحله است: *ارزیابی اولیه*، *داوری تخصصی* و *مصاحبه با شورا*.`,
      cites: [
        { title: "آیین‌نامه‌ی پذیرش", loc: "regulations/admission-v3.pdf · ماده ۲", score: "0.95" },
        { title: "فرایند پذیرش واحدها", loc: "regulations/process-flow.pdf · ص ۱", score: "0.88" }
      ]
    },
    "استقرار": {
      md: `طبق *ماده‌ی ۷ آیین‌نامه‌ی استقرار*، حداکثر مدت استقرار یک واحد فناور در مرکز رشد به این صورت است:

- **دوره‌ی پیش‌رشد**: تا **۹ ماه** (قابل تمدید یک‌بار به‌مدت ۳ ماه)
- **دوره‌ی رشد**: تا **۳ سال** (با امکان تمدید سالانه پس از ارزیابی)
- **دوره‌ی پسارشد**: حداکثر **۲ سال** برای واحدهای موفق در گذار به بازار

در پایان هر دوره، ارزیابی عملکرد توسط شورای مرکز انجام می‌شود.`,
      cites: [
        { title: "آیین‌نامه‌ی استقرار", loc: "regulations/residency.pdf · ماده ۷", score: "0.93" },
        { title: "جدول مدت‌زمان دوره‌ها", loc: "regulations/durations-table.pdf · ص ۲", score: "0.86" }
      ]
    },
    "تسهیلات مالی": {
      md: `تسهیلات مالی قابل ارائه به واحدهای فناور مستقر در مرکز شامل موارد زیر است:

- **وام قرض‌الحسنه** تا سقف **۵۰۰ میلیون تومان** با بازپرداخت ۳۶ ماهه
- **کمک‌هزینه‌ی توسعه‌ی محصول** برای واحدهای منتخب پس از داوری
- معافیت از پرداخت اجاره‌ی فضای کاری در ۶ ماه نخست دوره‌ی رشد
- پوشش بخشی از هزینه‌های شرکت در نمایشگاه‌ها و رویدادهای داخلی

ارائه‌ی تسهیلات منوط به *تأیید گزارش پیشرفت سه‌ماهه* است.`,
      cites: [
        { title: "آیین‌نامه‌ی حمایت‌های مالی", loc: "regulations/financial-aid.pdf · ماده ۴", score: "0.92" },
        { title: "سقف تسهیلات سال جاری", loc: "regulations/limits-1403.pdf · ص ۱", score: "0.89" },
        { title: "فرایند درخواست وام", loc: "regulations/loan-process.pdf · ص ۳", score: "0.78" }
      ]
    },
    "ارزیابی": {
      md: `ارزیابی دوره‌ای واحدهای مستقر **هر سه ماه یک‌بار** و طبق چارچوب زیر انجام می‌شود:

- ارائه‌ی **گزارش پیشرفت کتبی** توسط مدیر واحد
- جلسه‌ی *حضوری* با کارگروه ارزیابی مرکز
- بررسی شاخص‌های کلیدی: دستاوردهای فنی، توسعه‌ی تیم، جذب سرمایه و درآمد

در صورت کسب امتیاز کمتر از حد نصاب در دو دوره‌ی متوالی، پرونده‌ی واحد به شورا ارجاع می‌شود.`,
      cites: [
        { title: "آیین‌نامه‌ی ارزیابی", loc: "regulations/evaluation.pdf · ماده ۱۲", score: "0.94" },
        { title: "فرم گزارش سه‌ماهه", loc: "regulations/quarterly-form.pdf · ص ۱", score: "0.81" }
      ]
    }
  }
};
...
const GENERIC = {
  resume: {
    md: `این سؤال در محدوده‌ی پایگاه دانش رزومه‌های مرکز قرار می‌گیرد. در حال جست‌وجو در اسناد مرتبط هستم.

برای دریافت پاسخ دقیق‌تر، نام **حوزه‌ی تخصصی** یا **سطح تجربه‌ی موردنظر** را مشخص کنید. به‌عنوان مثال:

- "اعضای متخصص در **حوزه‌ی بلاک‌چین**"
- "توسعه‌دهندگان با **بیش از ۳ سال تجربه** در پایتون"`,
    cites: [
      { title: "نمایه‌ی اعضا — سال ۱۴۰۳", loc: "members/index.csv", score: "0.62" }
    ]
  },
  rules: {
    md: `سؤال شما در حوزه‌ی آیین‌نامه‌های مرکز قرار می‌گیرد. برای بازیابی دقیق‌تر پاسخ، می‌توانید موضوع را *مشخص‌تر* بپرسید:

- "شرایط **پذیرش** در دوره‌ی پیش‌رشد چیست؟"
- "**تسهیلات مالی** مرکز شامل چه مواردی است؟"
- "**مدت زمان** هر دوره چقدر است؟"`,
    cites: [
      { title: "فهرست آیین‌نامه‌ها", loc: "regulations/index.md", score: "0.58" }
    ]
  }
};

function pickResponse(botId, text) {
  const map = RESPONSES[botId] || {};
  for (const key of Object.keys(map)) {
    if (text.includes(key)) return map[key];
  }
  return GENERIC[botId];
}
